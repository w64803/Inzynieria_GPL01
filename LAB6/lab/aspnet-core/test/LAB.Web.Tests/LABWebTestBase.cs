...
{
    public abstract class LABWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static LABWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(LABWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<LABDbContext> action)
        {
            using (var context = IocManager.Resolve<LABDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<LABDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<LABDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<LABDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<LABDbContext, Task<T>> func)
        {
            T result;
